...
namespace ThingsBook.BusinessLogic
{
    public class LendsBL : BaseBL, ILendsBL
    {
        ...
        {
            await Data.Lends.CreateLend(thingId, lend);
        }

        public async Task Delete(Guid thingId, DateTime returnDate)
        {
            var thing = await Data.Things.GetThing(thingId);
            var delLend = Data.Lends.DeleteLend(thingId);
            var hist = MakeLendHistorical(thing, returnDate);
            var createHist = Data.History.CreateHistLend(hist);
            await Task.WhenAll(delLend, createHist);
        }

        public async Task DeleteHistoricalLend(Guid id)
        {
            await Data.History.DeleteHistLend(id);
        }

        public async Task<HistoricalLend> GetHistoricalLend(Guid id)
        {
            return await Data.History.GetHistLend(id);
        }
        ...
        public async Task<FilteredLends> GetThingLends(Guid userId, Guid thingId)
        {
            ...
            var thing = Data.Things.GetThing(thingId);            
            var hist = Data.History.GetThingHistLends(thingId);
        ...
        {
            await Data.Lends.UpdateLend(thingId, lend);
        }
        ...
        private LendBL LendBLFromThing(Thing thing)
        {
            ...
            var friend = Data.Friends.GetFriend(thing.Lend.FriendId);
            friend.Wait();
