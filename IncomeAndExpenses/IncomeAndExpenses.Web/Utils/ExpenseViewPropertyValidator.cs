            ...
            if (container is ExpenseViewModel expense)
            {
                switch (Metadata.PropertyName)
                {
                    case nameof(ExpenseViewModel.Amount):
                        ...
                        {
                            return new ModelValidationResult[]{ new ModelValidationResult { MemberName = string.Empty, Message="Amount shuld be greater than 0 and less than 99999999,99"}};
                        }
                        break;
                    case nameof(ExpenseViewModel.Date):
                        if (expense.Date == null)
                        {
                            return new ModelValidationResult[]{new ModelValidationResult { MemberName = string.Empty, Message="Date required"}};
                        }
                        if (expense.Date > DateTime.Today)
                        {
                            return new ModelValidationResult[] { new ModelValidationResult { MemberName = string.Empty, Message = "Should be less or equal than today" } };
                        }
                        break;
                }
            }
